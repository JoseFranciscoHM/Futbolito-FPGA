...
PROCESS
BEGIN
	WAIT UNTIL(clock_33Mhz'EVENT) AND (clock_33Mhz='1');

--Generate Horizontal and Vertical Timing Signals for Video Signal
-- H_count counts pixels (640 + extra time for sync signals)
-- 
--  Horiz_sync  ------------------------------------__________--------
--  H_count       0                640             650       746    799
--
	IF (h_count = 799) THEN
   		h_count <= "0000000000";
	ELSE
...
--Generate Horizontal Sync Signal using H_count
	IF (h_count <= 746) AND (h_count >= 650) THEN
 	  	horiz_sync <= '0';
	ELSE
 	  	horiz_sync <= '1';
	END IF;
...
-- Generate Vertical Sync Signal using V_count
	IF (v_count <= 492) AND (v_count >= 491) THEN
   		vert_sync <= '0';
	ELSE
  		vert_sync <= '1';
	END IF;
